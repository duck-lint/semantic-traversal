...
class CatalogAdmissionTests(unittest.TestCase):
    ...
    def test_real_completed_catalog_is_admitted_byte_for_byte(self):
        path = Path(r"F:\PROJECT-REPOS\semantic-traversal\out\capability_catalog.json")
        artifact = load_capability_catalog(path)
        self.assertEqual(artifact.sha256, "sha256:f636afb95099edca23b1e14257fc9007dc55f307197d1fa2416ae6cbdb974938")
        self.assertEqual(artifact.text, path.read_bytes().decode("utf-8"))
