...
- Preserved lexical SQLite retrieval as diagnostic instrumentation rather than treating it as a semantic gatekeeper.
...
- `python -m semantic_traversal.probes probe_lexical_retrieval_fixture_hit --repo-root . --data-root $env:TEMP\semantic-traversal-probes-hit` -> pass (`coverage_decision: blocked`)
- `python -m semantic_traversal.probes probe_lexical_retrieval_no_index --repo-root . --data-root $env:TEMP\semantic-traversal-probes-noindex` -> pass (`coverage_decision: blocked`)
- `python -m semantic_traversal.probes probe_lexical_retrieval_no_match --repo-root . --data-root $env:TEMP\semantic-traversal-probes-nomatch` -> pass (`coverage_decision: blocked`)
- `python -m semantic_traversal.probes probe_lexical_retrieval_no_query_terms --repo-root . --data-root $env:TEMP\semantic-traversal-probes-noquery` -> pass (`coverage_decision: blocked`)
...
- `python -m semantic_traversal.probes probe_blocked_runtime_with_disabled_extraction --repo-root . --data-root $env:TEMP\semantic-traversal-probes-extract-disabled` -> pass (`coverage_decision: blocked`, extraction statuses `disabled`)
...
- Lexical retrieval observations still persist diagnostically when extraction is disabled, but they do not approve synthesis.
